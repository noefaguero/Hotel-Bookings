...
class BookingsView extends View {
    ...
    public function build($bookingsList, $msg=null) {
        ...
        if ($msg) {
            $main .= 
            '<div class="alert bg-orange text-center p-5 m-3 rounded-5 alert-dismissible fade show" role="alert">
                <p>' . $msg . '</p>
                <button type="button" class="btn-close p-5 opacity-100" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>';
        }

        $main .= $bookingsList ? join($bookingsList) : '<p class="fw-bold p-3">ACTUALMENTE NO TIENES RESERVAS</p>';
